...
struct Trainer
{
#define DATA_LENGTH 1024                //expand every 1-4 weeks 2000
#define INFO_LENGTH 512                 //expand every 1-4 weeks 1000
#define NUMBER_OF_INPUT_NEURONS 640     //expand every 1-4 weeks 1000
#define NUMBER_OF_OUTPUT_NEURONS 640    //expand every 1-4 weeks 1000
#define MAX_INPUT_DURATION 10           //expand every 1-4 weeks 20
#define MAX_OUTPUT_DURATION 10          //expand every 1-4 weeks 20
#define SOLUTION_THRESHOLD 600          //change every week, reduce threshold to increase solution chance, 512 likely to give 50% chance to find solution.
    ...
    void initialize()
    {
        ...
        randomSeed[0] = 28; 	//change every week
        randomSeed[1] = 0;		//change every week
        randomSeed[2] = 7;		//change every week
        randomSeed[3] = 69;		//change every week
        randomSeed[4] = 255;	//change every week
        randomSeed[5] = 0;		//change every week
        randomSeed[6] = 73;		//change every week
        randomSeed[7] = 77;		//change every week
    ...
    bool findSolution(unsigned char nonce[32])
    {
        ...
        struct
        {
            char input[(NUMBER_OF_INPUT_NEURONS + INFO_LENGTH) * (DATA_LENGTH + NUMBER_OF_INPUT_NEURONS + INFO_LENGTH)];    //1152 * 2176 * sizeof(char) = 2,506,752 Bytes
            char output[(NUMBER_OF_OUTPUT_NEURONS + DATA_LENGTH) * (INFO_LENGTH + NUMBER_OF_OUTPUT_NEURONS + DATA_LENGTH)]; //1664 * 2176 * sizeof(char) = 3,620,864 Bytes
            unsigned int lengths[MAX_INPUT_DURATION * (NUMBER_OF_INPUT_NEURONS + INFO_LENGTH) + MAX_OUTPUT_DURATION * (NUMBER_OF_OUTPUT_NEURONS + DATA_LENGTH)]; //(10 * (640 + 512) + 10 * (640 + 1024)) * sizeof(unsigned int) = 112,640 Bytes
        } synapses; //6.24MB, initial memory 8388608 per thread
        ...
        random(computorPublicKey, nonce, (unsigned char*)&synapses, sizeof(synapses)); //initialize synapses
        for (unsigned int inputNeuronIndex = 0; inputNeuronIndex < NUMBER_OF_INPUT_NEURONS + INFO_LENGTH; inputNeuronIndex++) //outer loop iterates over each "Input" neuron
        {
            for (unsigned int anotherInputNeuronIndex = 0; anotherInputNeuronIndex < DATA_LENGTH + NUMBER_OF_INPUT_NEURONS + INFO_LENGTH; anotherInputNeuronIndex++)
            {
                ...
                synapses.input[offset] = (((unsigned char)synapses.input[offset]) % 3) - 1; //synapses.input will be -1,0,1
            }
        }
        for (unsigned int outputNeuronIndex = 0; outputNeuronIndex < NUMBER_OF_OUTPUT_NEURONS + DATA_LENGTH; outputNeuronIndex++) //outer loop iterates over each neuron
        {
            //For each neuron, it iterates over all its connections, where each connection corresponds to another neuron in the output section (the total number of connections being 2176
        ...
        //The loops above transform synapses.input and output value to one of the three values: -1, 0, or 1
        //The synapse value will only do multiplication with neuron value (1 or -1), only 2 bits of storage needed for each synapse value, 5m bits for input, 7m bits for output
        //The following loop is to set certain synapse values to 0 where the synapse is connecting the neuron with itself.
        ...
        memcpy(&neurons.input[0], &data, sizeof(data));//data can be simplified to 1 or -1 and remain constant in the following two loops
        memset(&neurons.input[sizeof(data) / sizeof(neurons.input[0])], 0, sizeof(neurons) - sizeof(data));
        /////////////////////////////////////////////FPGA START////////////////////////////////////////////////////////////////////////////////////////////////
        for (unsigned int tick = 0; tick < MAX_INPUT_DURATION; tick++) //representing the discrete time steps of the simulation up to MAX_INPUT_DURATION
        {
            unsigned short neuronIndices[NUMBER_OF_INPUT_NEURONS + INFO_LENGTH];
            unsigned short numberOfRemainingNeurons = 0;
            //Each neuron updates its value based on the values of other neurons and the synaptic weights between them.
            for (numberOfRemainingNeurons = 0; numberOfRemainingNeurons < NUMBER_OF_INPUT_NEURONS + INFO_LENGTH; numberOfRemainingNeurons++) //1152
            {
                neuronIndices[numberOfRemainingNeurons] = numberOfRemainingNeurons;
            }
            while (numberOfRemainingNeurons)
            {
                const unsigned short neuronRandomIndex = synapses.lengths[lengthIndex++] % numberOfRemainingNeurons; //Determine the neurons updating sequence, in a pre-determined random way
                const unsigned short inputNeuronIndex = neuronIndices[neuronRandomIndex];
                neuronIndices[neuronRandomIndex] = neuronIndices[--numberOfRemainingNeurons];
                //This selected neuron is then removed from the neuronIndices array by replacing its position with the last element of the array 
                for (unsigned short anotherInputNeuronIndex = 0; anotherInputNeuronIndex < DATA_LENGTH + NUMBER_OF_INPUT_NEURONS + INFO_LENGTH; anotherInputNeuronIndex++) //2176
                {
                    int value = neurons.input[anotherInputNeuronIndex] >= 0 ? 1 : -1; //if the value is non-negative, it assigns value as 1, otherwise it assigns -1.
                    value *= synapses.input[inputNeuronIndex * (DATA_LENGTH + NUMBER_OF_INPUT_NEURONS + INFO_LENGTH) + anotherInputNeuronIndex]; //Multiplying the Value by the Synapse Weight, (1 or -1) * (1 or 0 or 1)
                    neurons.input[DATA_LENGTH + inputNeuronIndex] += value; //XOR operations with counter
                }
            }
        }

        memcpy(&neurons.output[0], &neurons.input[DATA_LENGTH + NUMBER_OF_INPUT_NEURONS], INFO_LENGTH * sizeof(neurons.input[0])); //copy info part to output layer

        for (unsigned int tick = 0; tick < MAX_OUTPUT_DURATION; tick++)
        {
            unsigned short neuronIndices[NUMBER_OF_OUTPUT_NEURONS + DATA_LENGTH];
            unsigned short numberOfRemainingNeurons = 0;
            for (numberOfRemainingNeurons = 0; numberOfRemainingNeurons < NUMBER_OF_OUTPUT_NEURONS + DATA_LENGTH; numberOfRemainingNeurons++) //1664
            {
                neuronIndices[numberOfRemainingNeurons] = numberOfRemainingNeurons;
            }
            while (numberOfRemainingNeurons)
            {
                const unsigned short neuronRandomIndex = synapses.lengths[lengthIndex++] % numberOfRemainingNeurons;
                const unsigned short outputNeuronIndex = neuronIndices[neuronRandomIndex];
                neuronIndices[neuronRandomIndex] = neuronIndices[--numberOfRemainingNeurons];
                for (unsigned int anotherOutputNeuronIndex = 0; anotherOutputNeuronIndex < INFO_LENGTH + NUMBER_OF_OUTPUT_NEURONS + DATA_LENGTH; anotherOutputNeuronIndex++)
                {
        ...
        {
            if ((data[i] >= 0) == (neurons.output[INFO_LENGTH + NUMBER_OF_OUTPUT_NEURONS + i] >= 0))    // Compare the signs of the New_data and Original_data. 
            {
                score++;                                                                               //If match, means the network's prediction for that particular data point is correct
            }
        }
        /*printf("Original Data vs. Neurons Output:\n");
        for (unsigned int i = 0; i < DATA_LENGTH; i++)
        {
            printf("Data[%d]: %d | Neuron Output: %d\n", i, data[i], neurons.output[INFO_LENGTH + NUMBER_OF_OUTPUT_NEURONS + i]);
        }
        printf("\score: %i\n", score);*/
        /////////////////////////////////////////////FPGA END//////////////////////////////////////////////////////////////////////
...
static volatile long long numberOfTrainingIterations = 0;
...
static DWORD WINAPI trainingThreadProc(LPVOID)
{
    Trainer trainer;
    trainer.initialize();
    trainer.setComputorPublicKey(computorPublicKey);

    unsigned char nonce[32];
    while (!state)
    {
        if (trainer.findSolution(nonce))
        {
        ...
        _InterlockedIncrement64(&numberOfTrainingIterations);
    }
...
int main(int argc, char* argv[])
{
    if (argc < 3)
    {
        printf("Usage:   Trainer IP-address Id [Number of threads]\n");
        //use 192.127.0.1 or any IP for test, it will fail to send solution, ID is 60-letter 
        //example: trainer 136.243.41.109 BVIKTWXQPCEWTBVVHNHWYLTMFBTBAYKHOTOTITGLFAGHUHAFLDXTFIZEQTLO 2
    }
    else
    {
        printf("Trainer %d.0 is launched.\n", EPOCH);
        ...
        else
        {
            ...
            {
                CreateThread(NULL, 8388608, trainingThreadProc, 0, 0, NULL);
            }
            ...
            long long prevNumberOfTrainingIterations = 0;
            while (!state)
            {
                ...
                if (delta >= 1000)
                {
                    ...
                    printf("|   %d-%d%d-%d%d %d%d:%d%d:%d%d   |   %d it/s   |   %d solutions   |   %.10s...   |\n", systemTime.wYear, systemTime.wMonth / 10, systemTime.wMonth % 10, systemTime.wDay / 10, systemTime.wDay % 10, systemTime.wHour / 10, systemTime.wHour % 10, systemTime.wMinute / 10, systemTime.wMinute % 10, systemTime.wSecond / 10, systemTime.wSecond % 10, (numberOfTrainingIterations - prevNumberOfTrainingIterations) * 1000 / delta, numberOfFoundSolutions, argv[2]);
                    prevNumberOfTrainingIterations = numberOfTrainingIterations;
        ...
        printf("Trainer %d.0 is shut down.\n", EPOCH);
    }
